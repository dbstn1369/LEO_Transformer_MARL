...
def plot_convergence():
    """Single-panel convergence plot: Proposed vs GRLR vs MADRL.

    Shows training reward (delivery rate) with raw scatter + smoothed mean
    + confidence band, for all three learning-based schemes.
    """
    configs = [
        ("Proposed", "logs/train_rewards.npy", "#e41a1c", "-"),
        ("GRLR",     "logs/grlr_rewards.npy",  "#4daf4a", "-."),
    ...
    for label, path, color, ls in configs:
        ...
        ax.scatter(ep, raw, color=color, alpha=0.05, s=4, zorder=1)
        ax.fill_between(ep, lower, upper, color=color, alpha=0.13, zorder=2)
