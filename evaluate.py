...
def extract_transformer_weights(actor):
    """Extract effective w_v (β) and w_d from Transformer layers (Eq. 27).

    The model applies F.softplus(·) to β and w_d at forward time to enforce
    positivity, so the *effective* bias used by the policy is softplus(raw).
    We extract the effective values here so the Dijkstra edge-weight formula
    matches the policy's actual link-quality penalty.
    """
    import torch.nn.functional as F
    betas, w_ds = [], []
    for name, param in actor.named_parameters():
        if "attn.beta" in name:
            betas.append(F.softplus(param).item())
        elif "attn.w_d" in name:
            w_ds.append(F.softplus(param).item())
