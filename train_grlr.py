"""
Train the GRLR baseline (GAT actor with outage probability) with PPO-CTDE.
Saves to checkpoints/best_grlr.pt and logs/grlr_rewards.npy.

Usage:
    python train_grlr.py [--episodes N] [--device cpu|cuda]
"""
...
def main():
    ...
    parser.add_argument("--seed",     type=int,   default=cfg.SEED + 2)
    ...
    parser.add_argument("--epochs",   type=int,   default=cfg.PPO_EPOCHS)
    ...
    # GRLR: outage prob is feature only, not in attention score → partial blindness
    ...
    if args.resume and os.path.exists("checkpoints/best_grlr.pt"):
        ckpt = torch.load("checkpoints/best_grlr.pt", map_location=args.device)
        actor.load_state_dict(ckpt["actor"])
        critic.load_state_dict(ckpt["critic"])
        best_reward = ckpt.get("reward", -math.inf)
        if os.path.exists("logs/grlr_rewards.npy"):
            reward_history = np.load("logs/grlr_rewards.npy").tolist()
        print(f"[GRLR] Resumed from ep {ckpt.get('episode', '?')}, best={best_reward:.2f}")

    print(f"[GRLR] Training {args.planes}×{args.sats}={n_sats} agents "
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if ep_reward > best_reward:
            ...
            torch.save({
                ...
                "reward" : best_reward,
            }, "checkpoints/best_grlr.pt")

    np.save("logs/grlr_rewards.npy", np.array(reward_history))
    print(f"\n[GRLR] Done. Best reward: {best_reward:.2f}")
