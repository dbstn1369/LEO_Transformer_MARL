...
### `backup_paper_snapshot_20260506/` (most recent — full snapshot)

Full snapshot of code, data, figures, and tex taken on 2026-05-06 before
the hyperparameter-sweep redo. Contents:

- `code/` — all plot/eval/train scripts + `config.py` + `run_hp_sweep.py`
- `data/` — small/large/ablation CSVs (delay + PLR)
- `figures/` — 6 figures × {.png, .eps}
- `logs/` — 800-ep trained reward histories
- `tex/` — section5_simulation.tex + PROJECT_GUIDE.md + REVIEWER_DEFENSE_TODO.md

If anything goes wrong this week, restore from here:
```bash
cp backup_paper_snapshot_20260506/code/*.py .
cp backup_paper_snapshot_20260506/data/*.csv data/
cp backup_paper_snapshot_20260506/figures/*.{png,eps} figures/
cp backup_paper_snapshot_20260506/tex/* .
```

### `backup_before_hp_redo_20260506/` (HP-sweep specific)

Just the old `fig_convergence_hp` materials (the figure was using
inconsistent composite-reward y-axis; replacing with delivery-rate-based
sweep). Contains the old plot_paper_figures.py + the 5 old hyperparameter
.npy files + the old figure.
